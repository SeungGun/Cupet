...
def main(sock):
    while True:
        ...
        if str(translation).find('안') >= 0 or str(translation).find('녕') >= 0 or str(translation).find("야") >= 0:
            animal_said = random.choice(hello)
        elif str(translation).find('배고') >= 0:
            animal_said = random.choice(hungry)
        elif str(translation).find('졸') >= 0 or str(translation).find('피곤') >= 0:
            animal_said = random.choice(tired)
        else:
            animal_said = '잘모르겠어요.'

        print('메세지를 입력하시오 : ', animal_said)
        sock.sendall(animal_said.encode('utf-8'))
